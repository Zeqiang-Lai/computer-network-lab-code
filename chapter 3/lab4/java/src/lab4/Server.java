...
public class Server extends TransmitInfo{
	...
	private static String[] message = {"111111", 
								"1000111",
								"1010101"}; 
...
////				}
	...
	private static void printSendInfo() {
		System.out.println("当前发送的帧号为 " +(getNextFrameToSend()-1) +",期待的确认序号为 " + (getAckExpected()) +", 下一发送帧号为 " + getNextFrameToSend() + ", 期待的对方下一帧号为 " + getNextFrameExpected() );
	}
...
//		System.out.println("the strbuff is "+ sb.toString());
		packet = new DatagramPacket(sb.toString().getBytes(), sb.length(), InetAddress.getLocalHost(), 8800);
//		}else{
